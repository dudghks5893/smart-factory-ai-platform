...
C5-4B1에서는 train 84장만 사용한 ModelOpt INT8 PTQ로 explicit Q/DQ ONNX 생성을 완료했습니다. C5-4B2에서는 이 exact Q/DQ artifact를 source로 strongly typed TensorRT INT8 engine build를 완료했습니다. Exact engine은 외부 evidence로 보존했습니다. C5-4C에서는 이 engine을 rebuild하지 않고 validation 28장 metrics-only characterization을 완료했습니다. PyTorch/INT8 prediction은 `19/19`, unmatched는 `0/0`, class agreement는 `1.0`이었고, same-session latency는 PyTorch FP32 `30.977 ms` 대비 TensorRT INT8 `27.640 ms`로 약 `1.121x` speedup이 관측됐습니다. C5-4D에서는 C5-4C 관측 이후 INT8 parity budget을 `confidence<=0.12`, `box IoU>=0.93`, `mask IoU>=0.93`, same-session speedup `>=1.05x`로 별도 policy에 freeze했습니다. C5-4E에서는 policy commit의 clean state에서 exact INT8 engine을 rebuild 없이 복원해 validation 28장 prospective verification을 새로 실행했고 `39/39` acceptance check를 모두 통과해 `TENSORRT_INT8_PARITY_ACCEPTED`를 확인했습니다. 같은 session에서 PyTorch FP32 GPU mean `32.657 ms`, TensorRT INT8 mean `28.274 ms`, speedup 약 `1.155x`가 관측됐으며 C5-4 TensorRT INT8 parity lifecycle은 `CLOSED` 상태입니다.
