"""C6-5D annotated DeepStream video evidence contracts."""
...
EXPECTED_EVIDENCE_ID = "c6_5d_deepstream_annotated_video_evidence_v1"
EXPECTED_RESULT_PREFIX = "C6_5D_ANNOTATED_VIDEO_RESULT="
EXPECTED_CONTAINER_LABEL = "c6_5d_annotated_video=1"
EXPECTED_RUNTIME_IMAGE = "c6-5d-annotated-video:verify"
EXPECTED_VIDEO_NAME = "c6_5d_deepstream_annotated_sample.avi"


# ADD 2026-09-06: MP4 mask-overlay probe → MODIFY 2026-09-06: MJPEG/AVI evidence encoder
def build_annotated_video_probe_source(
...
constexpr const char* kConfigPath = "/tmp/c6_5d_annotated_video_nvinfer.txt";
...
void mark_invalid(EvidenceState* state, const std::string& reason) {
  if (!state->invalid) {
    state->invalid = true;
...
GstPadProbeReturn inspect_pre_osd(
  ...
  auto* state = static_cast<EvidenceState*>(user_data);
  ...
  for (NvDsMetaList* frame_node = batch_meta->frame_meta_list;
    ...
    for (NvDsMetaList* object_node = frame_meta->obj_meta_list;
      ...
      if (mask.data == nullptr || mask.size == 0U ||
        ...
        mark_invalid(state, "object missing mask metadata");
        continue;
      }
      ...
      if (mask.size != expected_bytes) {
        mark_invalid(state, "mask byte-size mismatch");
        continue;
      }
      ...
      for (std::size_t index = 0; index < elements; ++index) {
        ...
        if (!std::isfinite(value) || value < 0.0F || value > 1.0F) {
          mark_invalid(state, "mask probability invalid");
          break;
        }
      }
...
int main(int argc, char** argv) {
  ...
  const std::string pipeline_text =
      ...
      "filesrc location=__SAMPLE_PATH__ ! h264parse ! nvv4l2decoder ! "
      "queue ! mux.sink_0 "
      "mux. ! nvinfer name=primary "
      "config-file-path=/tmp/c6_5d_annotated_video_nvinfer.txt ! "
      ...
      "jpegenc ! avimux ! "
      "filesink location=/evidence/c6_5d_deepstream_annotated_sample.avi "
      "sync=false";
  ...
  std::ifstream output_file(
      "/evidence/c6_5d_deepstream_annotated_sample.avi",
      std::ios::binary | std::ios::ate);
  const bool annotated_video_written =
      output_file.good() && output_file.tellg() > 0;

  const bool passed =
      !runtime_error && eos_observed && state.batch_meta_observed &&
      pre_frames_reached && post_frames_reached &&
      masks_cover_objects && state.mask_elements > 0U &&
      annotated_video_written && !state.invalid;
...
# ADD 2026-09-06: MP4 생성/검증 → MODIFY 2026-09-06: MJPEG/AVI 생성/재디코드 검증
def build_annotated_video_shell_payload(
    config: DeepStreamSegmentationConfig,
) -> str:
    source = build_annotated_video_probe_source(config)
    return "\n".join(
        (
            "set -euo pipefail",
            ...
            "gst-inspect-1.0 jpegenc >/dev/null",
            "gst-inspect-1.0 jpegdec >/dev/null",
            "gst-inspect-1.0 avimux >/dev/null",
            "gst-inspect-1.0 avidemux >/dev/null",
            "cat > /tmp/c6_5d_annotated_video_probe.cpp <<'C6_5D_CPP'",
            source,
            "C6_5D_CPP",
            ...
            "/tmp/c6_5d_annotated_video_probe.cpp "
            '-L"${DS}/lib" -Wl,-rpath,"${DS}/lib" '
            "-lnvdsgst_meta -lnvds_meta "
            "$(pkg-config --libs gstreamer-1.0) "
            "-o /tmp/c6_5d_annotated_video_probe",
            "set +e",
            "/tmp/c6_5d_annotated_video_probe",
            "status=$?",
            "set -e",
            "rm -f /tmp/c6_5d_annotated_video_probe.cpp "
            "/tmp/c6_5d_annotated_video_probe "
            "/tmp/c6_5d_annotated_video_nvinfer.txt",
            "if [ ${status} -eq 0 ]; then",
            f"  test -s /evidence/{EXPECTED_VIDEO_NAME}",
            "  gst-launch-1.0 -q filesrc "
            f"location=/evidence/{EXPECTED_VIDEO_NAME} ! "
            "avidemux ! jpegdec ! fakesink sync=false",
            "fi",
            "exit ${status}",
            "",
        )
...
# ADD 2026-09-06: Evidence container를 no-network/read-only-plan 경계로 고정한다.
def build_annotated_video_docker_command(
    plan_path: Path,
    evidence_dir: Path,
) -> tuple[str, ...]:
    return (
        ...
        "-v",
        f"{evidence_dir.resolve()}:/evidence:rw",
        "--entrypoint",
        "bash",
...
# ADD 2026-09-06: Container stdout의 single annotated-video result를 파싱한다.
def parse_annotated_video_payload(
    stdout: str,
    *,
    ...
    if len(matches) != 1:
        raise ValueError("C6-5D annotated-video result count must be exactly one.")

    raw: object = json.loads(matches[0])
    if not isinstance(raw, dict):
        raise ValueError("C6-5D annotated-video result must be object.")
    return cast(dict[str, Any], raw)


# ADD 2026-09-06: Annotated-video payload가 sample-only evidence 경계를 만족하는지 검증한다.
def validate_annotated_video_payload(payload: dict[str, Any]) -> None:
    expected_fields = {
        "status",
        "pre_osd_frames",
        "post_osd_frames",
    ...
    if set(payload) != expected_fields:
        raise ValueError("C6-5D annotated-video payload fields changed.")
    ...
    if any(type(payload[name]) is not int for name in integer_fields):
        raise TypeError("C6-5D annotated-video counters must be int.")

    if not isinstance(payload["status"], str):
        raise TypeError("C6-5D annotated-video status must be str.")
    if not isinstance(payload["invalid_reason"], str):
        raise TypeError("C6-5D annotated-video invalid_reason must be str.")
    ...
    if any(type(payload[name]) is not bool for name in (*true_flags, *false_flags)):
        raise TypeError("C6-5D annotated-video flags must be bool.")
    ...
    if (
        ...
        raise ValueError("C6-5D annotated-video runtime acceptance failed.")
